      ...
      <el-table-column label="餐桌状态" width="180">
        <template slot-scope="scope">
          <i
            ...
            :class="scope.row.foodtable_state ? 'eat' : ''"
          ></i>
          <span style="margin-left: 5px" v-if="scope.row.foodtable_state"
            >就餐中</span
          >
          <span style="margin-left: 5px" v-else>空桌</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async handleDelete(row) {
      // 删除按钮
      try {
        await this.$confirm(`确定要删除${row.foodtable_describe}餐桌吗`, {
          cancelButtonText: "取消",
