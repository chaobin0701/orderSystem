    ...
    <div class="works">
        ...
        <el-table :data="staffInfo" style="width: 100%" border>
            ...
            <el-table-column label="性别" width="180">
                <template slot-scope="scope">
                    <span style="margin-left: 5px">{{ scope.row.staff_sex }}</span>
                </template>
            </el-table-column>
        ...
        <ModifyStaff :dialogFormVisible.sync="dialogFormVisible" @queryStaff="queryStaff" :form="form" :title="title"
    ...
    methods: {
        ...
        async handleDelete(row) {  // 删除按钮
            try {
                ...
                // 确认删除
                await this.deleteMsg(row.staff_id)
                this.$message({
                    type: "success",
                    message: "删除成功!",
                });
        ...
        async queryStaff() {// 获取数据
            this.$store.dispatch("queryStaff")
        },
        async deleteMsg(value) { //删除数据
            console.log(value)
